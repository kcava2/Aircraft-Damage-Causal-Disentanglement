...
class CausalVAE(nn.Module):
    def __init__(self, nn_type='mask', name='vae', z_dim=20, z1_dim=5, z2_dim=4, channel=3, scale=None, inference=False, alpha=0.3, beta=1, initial=True):
        ...
        self.channel = channel # number of input channels (3 for RGB aircraft images)
        
        # Scale for normalizing labels: shape (z1_dim, 2) containing [mean, half_range]
        if scale is None:
            self.scale = np.array([[0.5, 0.5]] * z1_dim, dtype=np.float32)
        else:
            self.scale = scale
    ...
    def negative_elbo_bound(self, x, label, mask = None, sample = False, adj = None, alpha=0.3, beta=1, lambdav=0.001):
        ...
        q_m, q_v, skips = self.enc.encode(x.to(x.device))
        ...
        decoded_bernoulli_logits,x1,x2,x3,x4 = self.dec.decode_sep(z_given_dag.reshape([z_given_dag.size()[0], self.z_dim]), label.to(x.device), skips)
