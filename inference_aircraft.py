...
def encode_image(lvae, img_tensor: torch.Tensor, u: torch.Tensor, z1_dim: int, z2_dim: int):
    ...
    """
    with torch.no_grad():
        q_m, q_v = lvae.enc.encode(img_tensor)
    ...
    return z_given_dag, f_z1


def decode_z(lvae, z_flat: torch.Tensor, u: torch.Tensor):
    """Decode a flat z vector back to an image."""
    with torch.no_grad():
        recon, _, _, _, _ = lvae.dec.decode_sep(z_flat, u)
    return recon
...
def classify_image(
    ...
    if save_recon:
        recon = decode_z(lvae, z_given_dag, u)
...
def generate_counterfactuals(
    ...
    recon_orig = decode_z(lvae, z_given_dag, u)
    save_image(denorm(img_tensor),        f'{output_dir}/{stem}_original.png')
